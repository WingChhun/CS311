...
using namespace std;
#include "vstack.h" //include the stack header file
...
int  _tmain(int argc, _TCHAR* argv[])
{
	...
	if(expression.length() >12) //Assuming that the expressino is not longer than 12 characters
	{
	...
	else
	{
		...
		if(error != -1)
		{
			...
			{//check if Stack is empty, if it is, program success, inform user the final result of expression.
				...
				postfixstack.clearIt(); //clear rest of the stack
			}
			else
			{//Stack is not empty, incomplete expressino found
				cout <<"Incomplete expression was found." <<endl <<endl;

				cout <<"Displaying the stack " <<endl; //Test, display the remains of the stack
				postfixstack.displayAll();
			}
		}
	} //end of else
